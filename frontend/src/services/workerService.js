...
export async function uploadWorkerPhoto(workerId, photoData) {
    try {
        ...
        // Ensure multipart/form-data is used (override global JSON header)
        const response = await api.post(`/workers/${workerId}/photo-file`, formData, {
...
export async function getWorkersByPatientId(patientId) {
    try {
        const response = await api.get(`/auth/workers/patient/${patientId}`);
        const result = response.data;
        ...
        if (result.code === "0" && result.data) {
            ...
            throw new Error(result.msg || 'Failed to get workers by patient ID');
        }
    } catch (error) {
        if (error.response?.data?.msg) {
            throw new Error(error.response.data.msg);
            ...
            throw new Error('Failed to get workers by patient ID');
        }
    }
}
