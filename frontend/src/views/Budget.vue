...
const getWarningInfo = () => {
  return getBudgetWarnings(budgetData.value)
}

const getTotalUsed = () => {
  return calculateTotalUsed(budgetData.value)
}

const getTotalBalance = () => {
  return calculateTotalBalance(budgetData.value)
}

const getTotalUsagePercentage = () => {
  return calculateUsagePercentage(budgetData.value)
}
...
const handleBudgetAdjust = async () => {
  try {
    ...
    // Check if budget exists, if not create one first
    if (!budgetData.value.id) {
      console.log('No budget found, creating new budget first...')
      const newBudgetData = {
        totalBudget: budgetAdjustForm.value.newTotalBudget || 0,
      ...
      const createResponse = await createBudget(newBudgetData)
      if (createResponse?.data) {
        budgetData.value = createResponse.data
        console.log('New budget created:', createResponse.data)
    ...
    if (adjustTab.value === 'total') {
      ...
      if (budgetAdjustForm.value.newTotalBudget && budgetAdjustForm.value.newTotalBudget > 0) {
        ...
        if (response?.data) {
          ...
          // Reload budget data to ensure all calculations are up to date
          await loadBudgetData()
  ...
  if (addSubElementForm.value.categoryId && addSubElementForm.value.name && 
    ...
    try {
      ...
      if (category) {
        ...
        const response = await createBudgetSubElement(
          userInfo.data.patientId,
          addSubElementForm.value.categoryId,
          subElementData
        )
        if (response?.data) {
