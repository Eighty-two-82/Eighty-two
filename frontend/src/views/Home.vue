...
import { getMe, getShiftTimeSettings } from '@/services/userService'
...
const loadHomeData = async () => {
  try {
    ...
    if (userRole.value !== 'worker' && userInfo.data.patientId) {
      try {
        ...
        if (budgetResponse?.data) {
          // Store the complete budget data to ensure consistency with Budget.vue
    ...
    // Load schedule data for workers
    if (userRole.value === 'worker') {
      try {
        const scheduleResponse = await getSchedulesByWorker(userInfo.data.id)
        if (scheduleResponse?.data) {
          // Transform backend schedule data to frontend format
          const transformedSchedules = scheduleResponse.data.map(schedule => ({
            id: schedule.id,
            date: schedule.scheduleDate || schedule.date || new Date().toISOString().split('T')[0],
            shift: schedule.shiftType ? `${schedule.shiftType.charAt(0).toUpperCase() + schedule.shiftType.slice(1)} Shift` : 'Regular Shift',
            time: schedule.shiftStartTime && schedule.shiftEndTime ? `${schedule.shiftStartTime} - ${schedule.shiftEndTime}` : 'TBD',
    ...
    // Load shift time settings for all roles (to get manager's settings)
    try {
      // For now, we'll use the manager's ID to get shift time settings
      // In a real app, you might want to get the manager ID from the user's organization
      ...
      if (shiftTimeResponse?.data) {
        // Store shift time settings for use in the UI
...
const budgetData = ref({ totalBudget: 0, categories: [] })

// Use the same calculation logic as Budget.vue to ensure consistency
const totalBudget = computed(() => budgetData.value.totalBudget || 0)
const totalUsed = computed(() => calculateTotalUsed(budgetData.value))
const totalLeft = computed(() => calculateTotalBalance(budgetData.value))
const totalUsagePct = computed(() => calculateUsagePercentage(budgetData.value))
const budgetUsageColor = computed(() => getUsageColor(totalUsagePct.value))

const topCategoryWarnings = computed(() => {
  return getTopCategoriesByUsage(budgetData.value.categories, 3)
})
