            ...
            >
              <div style="display: flex; flex-direction: column; align-items: center; height: 100%; justify-content: space-between;">
                ...
                <div style="margin-top: 8px;">
                  ...
                  >
                    {{ (getWorkerName(worker) || 'W').charAt(0).toUpperCase() }}
                  </a-avatar>
                </div>
                
                <!-- Name -->
                <div style="font-size: 12px; color: #333; margin-bottom: 8px; line-height: 1.2; font-weight: 500;">
          ...
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'photo'">
              <a-avatar
                ...
                :alt="getWorkerName(record) || 'Worker'"
              >
                {{ (getWorkerName(record) || 'W').charAt(0).toUpperCase() }}
              </a-avatar>
            </template>
            ...
            <a-form-item label="Select morning shift workers">
              ...
              >
                ...
                <template #option="{ value, label }">
                  <div style="display: flex; align-items: center; gap: 8px;">
                    <a-avatar :size="20" :src="getWorkerById(value)?.photo || undefined">
          ...
          <a-form-item label="Upload Photo for Selected Worker">
              <div v-if="selectedWorkerForPhoto" style="margin-bottom: 16px; padding: 12px; background: #f6ffed; border-radius: 6px;">
                <div style="display: flex; align-items: center; gap: 12px;">
                  <a-avatar :size="40" :src="selectedWorkerForPhoto.photo || undefined">
                    {{ (selectedWorkerForPhoto.name || 'W').charAt(0).toUpperCase() }}
                  </a-avatar>
                  <div>
                    <div style="font-weight: bold;">{{ selectedWorkerForPhoto.name || 'Unknown Worker' }}</div>
                    <div style="font-size: 12px; color: #666;">{{ selectedWorkerForPhoto.workerId }}</div>
                  </div>
                </div>
              </div>
              ...
              >
                <div v-if="fileList.length === 0">
                  ...
                  <p class="ant-upload-text">
                    {{ selectedWorkerForPhoto ? `Upload photo for ${selectedWorkerForPhoto.name || 'Worker'}` : 'Select a worker first' }}
                  </p>
    ...
    >
      <div v-if="selectedWorkerDetail" style="padding: 20px;">
        ...
        <div style="display: flex; align-items: center; gap: 20px; margin-bottom: 24px; padding: 16px; background: #f6ffed; border-radius: 8px;">
          <a-avatar
ma a            :size="80"
            :src="selectedWorkerDetail.photo || undefined"
            :alt="selectedWorkerDetail.name || 'Worker'"
            style="border: 3px solid #52c41a;"
          >
            {{ (getWorkerName(selectedWorkerDetail) || 'W').charAt(0).toUpperCase() }}
          </a-avatar>
          <div style="flex: 1;">
            <h2 style="margin: 0 0 8px 0; color: #1890ff;">{{ getWorkerName(selectedWorkerDetail) }}</h2>
    ...
    >
      <div v-if="selectedWorkerDetail" style="margin-bottom: 20px;">
        <div style="display: flex; align-items: center; gap: 12px; padding: 12px; background: #f6ffed; border-radius: 6px;">
          <a-avatar :size="40" :src="selectedWorkerDetail.photo || undefined">
            {{ (getWorkerName(selectedWorkerDetail) || 'W').charAt(0).toUpperCase() }}
          </a-avatar>
          <div>
            <div style="font-weight: bold;">{{ getWorkerName(selectedWorkerDetail) }}</div>
            <div style="font-size: 12px; color: #666;">{{ selectedWorkerDetail.workerId }}</div>
          </div>
        </div>
      </div>
...
import { getMe, getUsersByRole, updateShiftTimeSettings } from '@/services/userService'
...
const morningShiftOptions = computed(() => {
  return availableWorkers.value.map(worker => ({
    value: worker.id,
    label: `${getWorkerName(worker)} (${worker.workerId || worker.id})`
  }))
})
...
// Daily schedules data - now loaded from API
...
const workerColumns = [
  ...
  },
  {
    ...
    key: 'workerId',
    width: 100,
    customRender: ({ record }) => record.workerId || record.id || 'N/A'
  },
  {
    title: 'Name',
    key: 'name',
    width: 150,
    customRender: ({ record }) => getWorkerName(record)
  },
  {
    title: 'Status',
    key: 'status',
...
const getWorkerName = (record) => {
  // Handle both User and Worker object formats
  if (record?.name) {
    return record.name
    ...
    return fullName || record.workerId || record.id || 'Unknown Worker'
  } else if (record?.workerId) {
    return `Worker ${record.workerId}`
  } else if (record?.id) {
    return `Worker ${record.id.slice(-6)}` // Show last 6 characters of ID
  }
...
const handleUploadWorkerPhoto = (worker) => {
  ...
  message.info(`Ready to upload photo for ${worker.name || 'Worker'}`)
}
...
const selectWorker = (worker) => {
  ...
  message.info(`Selected worker: ${worker.name || 'Worker'} (${worker.workerId})`)
}
  ...
  try {
    ...
    try {
      const scheduleData = {
        ...
        afternoonShiftWorkerIds: [], // Not used in current UI
        eveningShiftWorkerIds: scheduleForm.value.eveningShift,
    ...
    if (fileList.value.length > 0 && selectedWorkerForPhoto.value) {
      ...
      if (uploadedFile && uploadedFile.originFileObj) {
        try {
          ...
          // Call backend API to upload and persist photo
          const resp = await uploadWorkerPhotoAPI(selectedWorkerForPhoto.value.id, uploadedFile.originFileObj)
          const updatedWorker = resp?.data
          // Prefer backend-provided URL
          const persistedUrl = updatedWorker?.photo || updatedWorker?.photoUrl
          ...
          if (workerIndex !== -1) {
            workers.value[workerIndex].photo = persistedUrl || workers.value[workerIndex].photo
            workers.value[workerIndex].photoUrl = persistedUrl || workers.value[workerIndex].photoUrl
          }
          ...
          if (dailyWorkerIndex !== -1) {
            dailyWorkers.value[dailyWorkerIndex].photo = persistedUrl || dailyWorkers.value[dailyWorkerIndex].photo
            dailyWorkers.value[dailyWorkerIndex].photoUrl = persistedUrl || dailyWorkers.value[dailyWorkerIndex].photoUrl
          }

          // Persist cache so refresh keeps avatars
          try {
            localStorage.setItem('cachedWorkers', JSON.stringify(workers.value))
            localStorage.setItem('dailyWorkers', JSON.stringify(dailyWorkers.value))
          } catch (_) {}
          ...
          message.destroy()
          console.error('Failed to upload worker photo:', e)
          message.error('Failed to upload worker photo')
        }
      }
    }
...
const getFilteredDailyWorkers = () => {
  ...
  // Filter workers by their shift type
  return dailyWorkers.value.filter(worker => {
    ...
    return worker.shiftType === expectedShiftType
  })
}
...
const getShiftInfo = () => {
  ...
  const shiftInfo = {
    ...
    evening: {
      title: 'Evening Shift',
      description: `Provides evening care (${settings.night.start} - ${settings.night.end}), evening medications, and night monitoring`
    }
  }
...
const confirmShiftTimeSettings = async () => {
  ...
  try {
    ...
    const shiftTimeData = {
      morningShiftStart: shiftTimeForm.value.morningStart.format('HH:mm'),
      morningShiftEnd: shiftTimeForm.value.morningEnd.format('HH:mm'),
      afternoonShiftStart: shiftTimeForm.value.afternoonStart.format('HH:mm'),
      afternoonShiftEnd: shiftTimeForm.value.afternoonEnd.format('HH:mm'),
      eveningShiftStart: shiftTimeForm.value.nightStart.format('HH:mm'),
      eveningShiftEnd: shiftTimeForm.value.nightEnd.format('HH:mm')
    }
    
    // Save settings to backend
    await updateShiftTimeSettings(userId, shiftTimeData)
    ...
    console.error('Failed to save shift time settings:', error)
    message.error('Failed to save shift time settings. Please try again.')
  }
}
...
onMounted(async () => {
  ...
  try {
    ...
    try {
      localStorage.removeItem('cachedWorkers')
      localStorage.removeItem('dailyWorkers')
    ...
    if (workers.value.length === 0) {
      console.log('⚠️ No workers loaded, trying fallback...')
      // Force load workers using getUsersByRole as fallback
      try {
        const fallbackResponse = await getUsersByRole('worker')
        workers.value = fallbackResponse.data || []
        ...
        if (workers.value.length > 0) {
          console.log('✅ Fallback workers loaded:', workers.value.length)
        }
      } catch (e) {
    ...
    // Do not use localStorage fallback to avoid stale ids
  } finally {
    loading.value = false
  }
})
...
const loadWorkers = async () => {
  try {
    ...
    if (patientId.value) {
      try {
        response = await getWorkersByPatientId(patientId.value)
      } catch (error) {
    ...
    // Initialize available workers (all active/bound workers)
    availableWorkers.value = workers.value.filter(worker => 
      worker.status === 'active' || worker.status === 'bound' || worker.status === 'ACTIVE' || worker.status === 'BOUND'
    )
    
    console.log('Workers loaded:', workers.value.length, 'Available:', availableWorkers.value.length)
  ...
  try {
    if (organizationId.value) {
      ...
      const scheduledWorkers = schedules.map(schedule => {
        ...
        const localSettings = shiftTimeSettings.value || {}
        const defaults = {
          morning: { start: '06:00', end: '14:00' },
          afternoon: { start: '14:00', end: '22:00' },
          night: { start: '22:00', end: '06:00' }
        ...
        const settings = {
          morning: localSettings.morning || defaults.morning,
          afternoon: localSettings.afternoon || defaults.afternoon,
          night: localSettings.night || defaults.night
        }
        if (shiftType === 'morning') {
          shiftStartTime = settings.morning.start || '09:00'
          shiftEndTime = settings.morning.end || '14:00'
        } else if (shiftType === 'afternoon') {
          shiftStartTime = settings.afternoon.start || '16:00'
          shiftEndTime = settings.afternoon.end || '22:00'
        } else if (shiftType === 'evening') {
          shiftStartTime = settings.night.start || '22:00'
          shiftEndTime = settings.night.end || '06:00'
        } else {
        ...
        return {
          ...
          email: completeWorker?.email || '', // Get email from complete worker data
          shiftType: shiftType,
          shiftStartTime: shiftStartTime,
          shiftEndTime: shiftEndTime,
          status: schedule.status,
          notes: schedule.notes,
          photoUrl: schedule.workerPhotoUrl
        }
      })
    ...
    const cachedSchedules = localStorage.getItem('dailySchedules')
    const cachedDailyWorkers = localStorage.getItem('dailyWorkers')
    
    if (cachedSchedules) {
      try {
        dailySchedules.value = JSON.parse(cachedSchedules)
        console.log('Loaded daily schedules from cache')
      } catch (e) {
        console.error('Failed to parse cached schedules:', e)
      }
    }
    
    if (cachedDailyWorkers) {
      try {
        dailyWorkers.value = JSON.parse(cachedDailyWorkers)
        console.log('Loaded daily workers from cache')
      } catch (e) {
        console.error('Failed to parse cached daily workers:', e)
      }
    }
  }
}
