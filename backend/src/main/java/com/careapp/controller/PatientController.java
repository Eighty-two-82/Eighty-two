...
public class PatientController {
    ...
    @PostMapping("/{patientId}/remove-organization")
    public Result<String> removeOrganizationFromPatient(@PathVariable String patientId, @RequestBody Map<String, Object> removalData) {
        try {
            System.out.println("🔍 Backend - removeOrganizationFromPatient called for patientId: " + patientId);
            System.out.println("🔍 Backend - removalData: " + removalData);
            
            boolean success = patientService.removeOrganizationFromPatient(patientId, removalData);
            if (success) {
                System.out.println("✅ Backend - Organization removed successfully from patient: " + patientId);
                return Result.success("Organization removed successfully", "Organization removed from patient!");
            } else {
                System.out.println("❌ Backend - Failed to remove organization from patient: " + patientId);
                return Result.error("400", "Failed to remove organization from patient!");
            }
        } catch (Exception e) {
            System.err.println("❌ Backend - Error removing organization from patient: " + e.getMessage());
            return Result.error("500", "Failed to remove organization: " + e.getMessage());
        }
    }
}